...
;(function($){
    ...
    function ajaxComplete(status, xhr, settings) {
        var context = settings.context
        if(status.indexOf('timeout') > -1){
            settings.error.call(context, xhr, status, {name:'timeout',message:'timeout'})
        }
        else{
